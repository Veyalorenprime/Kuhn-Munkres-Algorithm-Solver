...
class main(tk.Frame):
    def __init__(self, parent):
        ...
        self.win1.geometry("450x50")
        self.win1.attributes('-topmost', 'true')
        btn = ttk.Button(self.win1, text="Créer", command=self.win1_submit)
        self.e1 = tk.Entry(self.win1, width = 5)
        self.lbl = tk.Label(self.win1, text = "Entrer la taille de la matrice")
    ...
    def win1_submit(self):
        ...
        self.solve = ttk.Button(self, text="Résoudre", width=8, command=self.on_submit)
    ...
    def on_submit(self):
        ...
        for i in range(solver.n):
            self.resultLabel["text"] += ("Groupe #"+str(sol[i][0]+1)+" --> Projet #"+str(sol[i][1]+1)+"\n")
